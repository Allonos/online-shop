...
import turnImage from './TurnTablesImage';
import { Link } from 'react-router-dom';

function TurnTablesBestsellers() {
  ...
  return (
    <motion.div className={classes['tv-carousel']} ref={carousel} whileTap={{ cursor: "grabbing" }}>
      ...
      >
        {turnImage.map(turnImage => {
          const { image, desc, price, id } = turnImage;
          ...
          return (
            <motion.div className={classes['tv-carousel-item']} key={id}>
              ...
              <Link to={`item/audio-system-10/audio`} className={classes['tv-first-button']}>Check it!</Link>
            </motion.div>
          )
        })}
      </motion.div>
...
export default TurnTablesBestsellers